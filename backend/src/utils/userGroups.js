...
// 完整权限组 (可访问所有功能)
const FULL_ACCESS_GROUPS = [UserGroup.SYSTEM, UserGroup.ADMIN_A, UserGroup.ADMIN_B, UserGroup.TEACHER]

// 有限权限组 (只能录入特定类型记录)
const LIMITED_ACCESS_GROUPS = [UserGroup.FACULTY]

// 最小权限组 (清洁人员)
const MINIMAL_ACCESS_GROUPS = [UserGroup.CLEANING]

// 日程访问权限组 (排除清洁人员)
const SCHEDULE_ACCESS_GROUPS = [
  UserGroup.SYSTEM,
  UserGroup.ADMIN_A,
  UserGroup.ADMIN_B,
  UserGroup.TEACHER,
...
// 检查是否有日程访问权限
function hasScheduleAccess(userGroup) {
...
// 检查是否有FAD执行权限
function hasFADExecutionAccess(userGroup) {
